...
import { sendOtpEmail, sendResetEmail, sendMigrationOtpEmail } from '../../utils/mailer.js';
...
export type LoginResult = { ok: true; token: string; refreshToken: string; user: { id: string; email: string; firstName: string | null; lastName: string | null; role: 'user' | 'admin' } } | { invalid: true } | { unverified: true } | { migrationRequired: true };
...
export async function loginUser(email: string, password: string, options?: { userAgent?: string; ipAddress?: string }): Promise<LoginResult> {
  ...
  if (user.migration) {
    ...
    try {
      ...
      logger.error({ email, err: error }, 'Failed to send migration OTP');
      throw error;
    }
    return { migrationRequired: true };
  }
...
export type MigrationVerifyResult = { ok: true } | { notFound: true } | { invalid: true } | { used: true } | { expired: true };

export async function verifyMigrationOtp(email: string, code: string): Promise<MigrationVerifyResult> {
  const user = await repo.getUserByEmail(email);
  if (!user) return { notFound: true };
  ...
  for (const otp of otps) {
    ...
    if (ok) {
      await repo.markOtpAsUsed(otp.id);
      logger.info({ email }, 'Migration OTP verified');
      return { ok: true };
    } else {
      await repo.incrementOtpAttempts(otp.id);
    }
...
export type MigrationPasswordResult = { ok: true } | { notFound: true } | { invalid: true };

export async function setMigrationPassword(email: string, password: string): Promise<MigrationPasswordResult> {
  const user = await repo.getUserByEmail(email);
  if (!user) return { notFound: true };
  ...
  // Update migration flag to false after password is set
  await repo.updateUserMigrationFlag(user.id, false);

  // Mark user as verified
  await repo.verifyUser(user.id);

  logger.info({ email }, 'Migration password set and user verified');
  return { ok: true };
}
