...
export async function login(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if ('migrationRequired' in result) return res.status(200).json({ success: true, message: 'Migration required. OTP sent to your email.', migrationRequired: true });
...
export async function migrationVerify(req: Request, res: Response, next: NextFunction) {
  try {
    const body = migrationVerifySchema.parse(req.body);
    const result = await service.verifyMigrationOtp(body.email, body.code);
    ...
    return res.json({ success: true, message: 'Migration verified. Please set your new password.' });
  } catch (e) {
    next(e);
  }
}

export async function migrationPassword(req: Request, res: Response, next: NextFunction) {
  try {
    const body = migrationPasswordSchema.parse(req.body);
    const result = await service.setMigrationPassword(body.email, body.password);

    if ('notFound' in result) return res.status(404).json({ error: 'User not found', errorMessage: 'Account not found' });
    if ('invalid' in result) return res.status(400).json({ error: 'Invalid request', errorMessage: 'Invalid request' });

    return res.json({ success: true, message: 'Password set successfully. You can now login with your new password.' });
  } catch (e) {
    next(e);
  }
}
